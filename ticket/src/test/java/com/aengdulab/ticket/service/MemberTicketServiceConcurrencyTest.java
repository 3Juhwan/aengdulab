...
import com.aengdulab.ticket.service.support.SynchronizedServiceWrapper;
                ...
                IntStream.range(0, MemberTicket.MEMBER_TICKET_COUNT_MAX)
                        .forEach(ticketCount ->
                                executorService.submit(() -> {
                                    try {
                                        SynchronizedServiceWrapper.execute(
                ...
                IntStream.range(0, ticketIssueCount)
                        .forEach(ticketCount -> {
                            ...
                            executorService.submit(() -> {
                                try {
                                    SynchronizedServiceWrapper.execute(
                                            () -> memberTicketService.issue(member.getId(), ticketId), member);
